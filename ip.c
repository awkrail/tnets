...
ip_iface_alloc(const char *unicast, const char *netmask)
{
  ...
  iface->broadcast = (iface->unicast & iface->netmask) | ~iface->unicast;
  return iface;
}
...
ip_output_device(struct ip_iface *iface, const uint8_t *data, size_t len, ip_addr_t dst)
{
  ...
  return net_device_output(NET_IFACE(iface)->dev, NET_POROTOCOL_TYPE_IP, data, len, &dst);
}

static ssize_t
ip_output_core(struct ip_iface *iface, uint8_t protocol, const uint8_t *data, size_t len,
               ip_addr_t src, ip_addr_t dst, uint16_t id, uint16_t offset)
{
...
ip_output(uint8_t protocol, const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst)
{
  ...
  if(src == IP_ADDR_ANY) {
    ...
    if(dst != IP_ADDR_BROADCAST && (iface->unicast & iface->netmask) != (dst & iface->netmask)) {
      errorf("dst addr could not be reached");
      return -1;
    }
  }
